...
{
	int result;
	assert( (intptr_t)parameter >= 0 && (intptr_t)( parameter ) + 1 < MAX_STATS );

	result = ( cg.predictedPlayerState.stats[(intptr_t)parameter] & 0xFFFF ) << 16;
	result |= ( cg.predictedPlayerState.stats[(intptr_t)(parameter)+1] & 0xFFFF );
	return result;
}
// !racesow
