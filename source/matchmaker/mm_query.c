...
static int StatQuery_Header( stat_query_t *query, const char *key, const char *value )
{
	if( !query->req )
		return -1;
...
static int StatQuery_CustomRequest( stat_query_t *query, const char *method )
{
	if( !query->req )
		return -1;
...
// racesow
static void StatQuery_PrepareJson( stat_query_t *query, stat_query_section_t *data )
{
	char *json_text;
	size_t jsonSize;
...
static void StatQuery_SendJson( stat_query_t *query, stat_query_section_t *data )
{
	if( query->url )
	{
